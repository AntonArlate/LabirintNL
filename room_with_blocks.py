...
class MyObject:
    ...
    def move(self,x,y):
        ...
        if self.wallContact(self.canvas.coords(self.oval)[0]+self.scale/2+x,
                            self.canvas.coords(self.oval)[1]+self.scale/2+y):
...
class Block:
    ...
    def __init__(self,x,y,w,h,a,color,canvas):
        global walls
        self.x=x
        self.y=y
        self.width=w
        ...
        self.canvas=canvas
        self.ang=a
        walls.append(DrawableLine(Point(x,y),Point(x+w,y),3,"red",self.canvas))
        walls.append(DrawableLine(Point(x+w,y),Point(x+w,y+h),3,"red",self.canvas))
        walls.append(DrawableLine(Point(x+w,y+h),Point(x,y+h),3,"red",self.canvas))
        walls.append(DrawableLine(Point(x,y+h),Point(x,y),3,"red",self.canvas))
...
class Room:
    ...
    def getDoors(self):
        ...
        for item in self.pre_walls:
            if len(item["wall"])==2:
                walls.append(DrawableLine(item["wall"][0],item["wall"][1],3,"red",self.canvas)) 
            else:
                for w in range(0,len(item["wall"]),2):
                    walls.append(DrawableLine(item["wall"][w],item["wall"][w+1],3,"red",self.canvas))        



room=Room(30,30,500,500,5,50,canvas)
block=Block(150,200,100,100,0,"green",canvas)
block=Block(350,400,100,100,0,"green",canvas)
